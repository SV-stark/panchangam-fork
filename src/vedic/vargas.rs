...
fn is_movable(sign_0: u8) -> bool {
    matches!(sign_0 % 3, 0)
} // Aries(0), Cancer(3)...
fn is_fixed(sign_0: u8) -> bool {
    matches!(sign_0 % 3, 1)
} // Taurus(1), Leo(4)...
...
fn calculate_d7(long: f64) -> VargaPosition {
    ...
    let start_sign_0 = if sign_0 % 2 == 0 {
        sign_0
    } else {
        (sign_0 + 6) % 12
    };
...
fn calculate_d9(long: f64, method: D9Variation) -> VargaPosition {
    ...
    match method {
        D9Variation::Parashara => {
            let start_offset = if is_movable(sign_0) {
                0
            } else if is_fixed(sign_0) {
                8
            }
...
fn calculate_d16(long: f64) -> VargaPosition {
    ...
    let start_sign = if is_movable(sign_0) {
        0
    } else if is_fixed(sign_0) {
        4
    } else {
...
fn calculate_d45(long: f64) -> VargaPosition {
    ...
    // Fixed: Leo(4)
    // Dual: Sagittarius(8)
    let start = if is_movable(sign_0) {
        0
    } else if is_fixed(sign_0) {
        4
    } else {
...
fn calculate_d249(long: f64) -> VargaPosition {
    ...
    // Start logic
    // Index 0 is Aries (Odd) -> Offset 0.
    // Index 1 is Taurus (Even) -> Offset 8 (9th from Taurus is Capricorn).
