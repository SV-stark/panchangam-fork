...
fn calculate_saptavargaja_bala(planet_id: i32, planets: &[PlanetInput]) -> f64 {
    ...
    for varga_type in vargas.iter() {
        ...
        let lord_sign_d1 = lord_data
            .map(|p| ((p.longitude / 30.0).floor() as u8 % 12) + 1)
            .unwrap_or(0);
...
pub fn calculate_shadbala_profile(
    ...
    for id in 0..7 {
        ...
        let uchcha = calculate_uchcha_bala(long, id);
        let saptavargaja = calculate_saptavargaja_bala(id, planets);
        let sthana = uchcha + saptavargaja;
        ...
        let kashta = ((60.0 - uchcha) * (60.0 - chesta)).sqrt();
